...
class PinEye {
  ...
  async dailyReward(token, proxy) {
    ...
    try {
      ...
      if (response.data && response.data.data && response.data.data.canClaim) {
        ...
        const claimResponse = await axios.post(claimUrl, null, this.getAxiosConfig(token, proxy));
  ...
  async checkAndBuyLottery(token, proxy) {
    ...
    try {
      ...
      if (!ticket.hasBuyed) {
        ...
        const buyResponse = await axios.post(buyTicketUrl, null, this.getAxiosConfig(token, proxy));
  ...
  async dailyCombo(token) {
    ...
    try {
      const response = await axios.post(url, null, this.getAxiosConfig(token, this.proxy));
  ...
  async claimPratice(token, proxy) {
    ...
    if (practices && practices?.length > 0) {
      ...
      if (taskDoing) {
        ...
        if (todayPractice) {
          ...
          const res = await this.claimPratice(token, proxy);
          if (res?.isSuccess) {
